...
public class ChiSoService {
    ...
    public Page<ChiSo> getTenantMetersByContracts(List<Long> hopDongIds, Integer page, Integer size,
                                                   String sortBy, String direction) {
        Pageable pageable = PageableUtils.build(page, size, sortBy, direction, "ky");
        if (hopDongIds == null || hopDongIds.isEmpty()) {
            return new PageImpl<>(List.of(), pageable, 0);
        }
        return chiSoRepository.findByHopDongIds(hopDongIds, pageable);
    }
    ...
    public Page<ChiSo> searchTenantMetersByContracts(List<Long> hopDongIds, String type, String period,
        ...
        ChiSo.Loai normalizedType = parseSearchType(type);
        String normalizedPeriod = period == null ? "" : period.trim().toLowerCase(Locale.ROOT);
        return chiSoRepository.searchByHopDongIds(hopDongIds, normalizedType, normalizedPeriod, pageable);
    }
