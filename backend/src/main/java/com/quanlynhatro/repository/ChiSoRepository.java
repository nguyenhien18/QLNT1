...
public interface ChiSoRepository extends JpaRepository<ChiSo, Long>, JpaSpecificationExecutor<ChiSo> {
    ...
    @Query("""
            select cs
            from ChiSo cs
            where cs.hopDong.hopDongId in :hopDongIds
            """)
    Page<ChiSo> findByHopDongIds(@Param("hopDongIds") List<Long> hopDongIds, Pageable pageable);
    ...
    @Query(value = """
            ...
            where cs.hopDong.hopDongId in :hopDongIds
              ...
              and (:period = '' or lower(coalesce(cs.ky, '')) like concat('%', :period, '%'))
            """)
    Page<ChiSo> searchByHopDongIds(@Param("hopDongIds") List<Long> hopDongIds,
                                   @Param("type") ChiSo.Loai type,
                                   @Param("period") String period,
                                   Pageable pageable);
}
